...
bool GenerateCppGRPC(const Parser &parser, const std::string &path,
                     const std::string &file_name) {
  std::vector<int> service_indexes;
  auto i = 0;
  for (ServiceDef *service : parser.services_.vec) {
    if (!service->generated) {
      service_indexes.push_back(i);
    }
    ++i;
  }
  if (service_indexes.empty()) return true;
  ...
  std::string header_code =
      ...
      grpc_cpp_generator::GetHeaderServices(&fbfile, generator_parameters, service_indexes) +
  ...
  std::string source_code =
      ...
      grpc_cpp_generator::GetSourceServices(&fbfile, generator_parameters, service_indexes) +
