 ...
 public:
  PythonGenerator(const Parser &parser, const std::string &path,
    ...
    static const char *const keywords[] = {
        ...
        "while", "with",   "yield",    "InitFromBuf", "InitFromObj", "Pack",
    };
  ...
  // structs.
  void
  StructBuilderArgs(const StructDef &struct_def, const std::string nameprefix,
                    const std::string namesuffix, bool has_field_name,
                    const std::string fieldname_suffix, std::string *code_ptr) {
  ...
  void InitializeFromBuf(const StructDef &struct_def, std::string *code_ptr) {
    ...
    code += GenIndents(2) + "_i = " + struct_name + "()";
    code += GenIndents(2) + "_i.Init(buf, pos)";
    code += GenIndents(2) + "return cls.InitFromObj(_i)";
    code += "\n";
  }
  ...
  void InitializeFromObjForObject(const StructDef &struct_def,
    ...
    code += GenIndents(1) + "@classmethod";
    code += GenIndents(1) + "def InitFromObj(cls, _i):";
    code += GenIndents(2) + "x = " + struct_name + "T()";
    code += GenIndents(2) + "x._UnPack(_i)";
  ...
  static std::string RealMakeCamel(const std::string &in, bool first = true) {
    std::string s;
    ...
    switch (type.base_type) {
      ...
      default:
        return "self._tab.Get(flatbuffers.number_types." +
               RealMakeCamel(GenTypeGet(type)) + "Flags, ";
    }
  }
  ...
  std::string GenMethod(const FieldDef &field) {
    return (IsScalar(field.value.type.base_type) || IsArray(field.value.type))
               ? RealMakeCamel(GenTypeBasic(field.value.type))
