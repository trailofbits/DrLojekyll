...
class CppGenerator : public BaseGenerator {
 ...
 private:
  ...
  void GenFieldTypeHelper(const StructDef &struct_def) {
    ...
    code_ += "  template<size_t _Index>";
    code_ += "  using FieldType = \\";
    code_ += "decltype(std::declval<type>().get_field<_Index>());";
  }
  ...
  void GenIndexBasedFieldGetter(const StructDef &struct_def) {
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      need_else = true;
      code_ += "if constexpr (_Index == {{FIELD_INDEX}}) \\";
      code_ += "return {{FIELD_NAME}}();";
    }
    code_ += "    else static_assert(_Index != _Index, \"Invalid Field Index\");";
    code_ += "  }";
  }
